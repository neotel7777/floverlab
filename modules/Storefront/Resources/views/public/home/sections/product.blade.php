<div class="product-card products-day__slide" data-id="{{$product->id}}" data-price="{{$product->pivot->price}}" data-flash_id="{{ ($product->pivot)?$product->pivot->flash_sale_id:0}}">
    ...
    <div class="product-card_description">
        ...
        <div class="product-card_price ">{{price_format(($product->pivot->price->amount()))}} MDL</div>
    </div>
    <button type="button" class="product-card__favorite" aria-label="{{ trans('storefront::layout.add_in_favorites') }}"
            data-id="{{$product->id}}"
            data-flash_id="{{ ($product->pivot)?$product->pivot->flash_sale_id:0}}">
